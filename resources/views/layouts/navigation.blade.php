                    ...
                    <a href="{{ route('cart.index') }}" class="text-gray-500 hover:text-gray-700">
                        ...
                        @auth
                            <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                                {{ auth()->user()->cart ? auth()->user()->cart->totalItems() : 0 }}
                            </span>
                        @else
                            @if(session('cart_session_id'))
                                <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                                    {{ Cart::where('session_id', session('cart_session_id'))->first() ? Cart::where('session_id', session('cart_session_id'))->first()->totalItems() : 0 }}
                                </span>
                            @endif
                        @endauth
                    </a>
